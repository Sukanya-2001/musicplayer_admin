...
export const AlbumTableRow: React.FC<OrderProps> = ({ row, refetch }) => {
  ...
  const { mutateAsync: deleteMutate, isPending: deletePending } =
    useDeleteAlbumHook(row?._id, { imageKey: row?.file });
  const { mutateAsync: statusMutate, isPending: statusChangePending } =
    usechangeStatusAlbumHook(row?._id);
  ...
  const handleEdit = () => {
    router.push(`/dashboard/album-edit/${row?._id}`);
  };
