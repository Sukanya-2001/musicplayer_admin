...
const DashboardWrapper: React.FC<DashBoardProps> = ({
  ...
  const drawerContent = (
    <div
      style={{ backgroundColor: "#FFFFFF", color: "#333333", height: "100%" }}
    >
      <Link
        href="/"
      ...
      {[sideFirstItems, sideSecondItems, sideThirdItems].map(
        (menuItems, index) => (
            ...
            <List>
              {menuItems.map((item) => (
                <ListItem key={item.name} disablePadding>
                  <ListItemButton
                    onClick={() => {
                      console.log(isLoggedIn, item?.name);

                      isLoggedIn && item?.name === "Logout"
                        ? handleLogout()
